...
class RealmManager {
    ...
    func getUserProfile(mail : String) -> Profile? {
            ...
            return realm.objects(Profile.self).filter(predicate)[0]
    }
    ...
    func getAllExercises(user: Profile) -> [ExercisesClass] {
        ...
        for name in user.listExercies {
                list.append(realm.objects(ExercisesClass.self).filter("name = \(name)")[0])
        }
    ...
    func getAllResult(mail: String) -> Results<ResultClass> {
        return realm.objects(ResultClass.self).filter("owner_mail = \(mail)")
    }
